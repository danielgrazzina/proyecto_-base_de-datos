...
def crear_base():
    ...
    cursor = conn.cursor()
    query = "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='producto'"
    ...
    query = "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='cliente'"
    exist = cursor.execute(query)
    if exist == 0:
        query = ('''CREATE TABLE cliente(
            CI_CLIENTE TEXT NOT NULL PRIMARY KEY,
            NOMBRE TEXT NOT NULL,
            APELLIDO TEXT NOT NUL,
            TELEFONO TEXT NOT NULL,
            DIRECCION TEXT NOT NULL,
            DEUDA REAL NOT NULL
            )''')
    query = "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='pedido'"
    exist = cursor.execute(query)
    if exist == 0:
        query = ('''CREATE TABLE pedido(
            ID_PEDIDO TEXT NOT NULL PRIMARY KEY AUTOINCREMENT,
            CI_CLIENTE TEXT NOT NULL,
            ID_PRODUCTO TEXT NOT NUL,
