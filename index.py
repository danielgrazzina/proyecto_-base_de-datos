...
class product:
    ...
    def windpedido1(self):
        ...
        self.bepedido = ttk.Button(self.windpedido, text = "Eliminar Pedido", width = 60, command =  self.editar_pedido_inventario)
        self.bepedido.place(x = 400, y = 450)
        self.bepedido['state'] = 'disable'

        self.bapedido = ttk.Button(self.windpedido, text = "Actualizar Pedido", width = 60, command =  self.editar_pedido_inventario)
    ...
    def agregar_pedido(self):
        if self.validacion_pedido():
            ...
            parametros = (str(self.ci.get()), str(self.id_pro.get()), self.cant.get(), self.dia())
            self.run_query(query, parametros)
            self.deuda_fun()
            messagebox.showinfo("BASE DE DATOS", "Datos guardados satisfactoriamente")
        else:
    ...
    "Funcion para aumentar la deuda"
    def deuda_fun(self):
    ...
    def editar_pedido_inventario(self):
        ...
        parametros = (self.id_pro.get(), self.cant.get(), self.dia(), self.ci_pedido_v)
        ...
        if self.cant.get() < self.cant_v1:
            if len(self.cant.get()) != 0:
                ...
                cursor.execute(query, parametros)
                self.total1 = (self.inventario_resta * self.valor_precio1) - self.valor_deuda1
        ...
        self.obt_pedidos()
        self.clean_pedido()
        Hovertip(self.ci, text = "No puede actualizar la CI de los pedidos ya ingresados", hover_delay = 360000)
    ...
    def seleccionar_click2(self, event):
        ...
        Hovertip(self.ci, text = "No puede actualizar la CI de los pedidos ya ingresados", hover_delay = 100)
